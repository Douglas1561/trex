...
gruponubes=new Group();
 ...
 piso=createSprite(300,270);
piso.addImage("correr",suelo)
  piso.scale=1
  piso.velocityX=-4
  suelo_invisible=createSprite(40,275,50,6);
  suelo_invisible.visible=false 
  
 dino=createSprite(50,245);
...
if (piso.x<0){
  ...
  trex_fin.visible=true
    ...
    grupocactus.setLifetimeEach(-1)
     text("puntuacion:"+puntajefinal,510,30)
...
function nubes(){
  if (frameCount % 70==0){
  nube=createSprite(570,Math.round(random(20,100)));
...
function obstaculos(){
  ...
  if (frameCount % 70==0){
  cactus=createSprite(570,Math.round(random(245,250)));
...
function marcador(){
  ...
  text("puntuacion:"+puntaje,510,30)
